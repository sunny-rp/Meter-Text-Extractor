...
function CameraPreview({ videoRef, isStreaming, error, onRetry }) {
  ...
  useEffect(() => {
    if (error && error.includes("Permission denied")) setPermissionDenied(true)
  }, [error])
  ...
  // Accept both a ref-callback function (recommended) or a {current} ref object
  const attachRef = useMemo(() => {
    if (typeof videoRef === "function") return videoRef
          ...
          {permissionDenied && (
            <div className="text-xs text-gray-400 mt-4">
              ...
              <ul className="text-left space-y-1">
                <li>• Check site permissions in your browser</li>
                <li>• Ensure you're on HTTPS</li>
                <li>• Try Incognito (extensions off)</li>
              </ul>
            </div>
          )}
        </div>
  ...
  if (!isStreaming) {
    return (
      <div className="flex flex-col items-center justify-center h-full bg-gray-900 text-white">
        ...
        <p className="text-sm" aria-live="polite">Starting camera...</p>
  ...
  return (
    <div className="relative h-full w-full overflow-hidden">
      <video
        ...
        // style={{ transform: "scaleX(-1)" }} // enable if you want mirroring
      />
